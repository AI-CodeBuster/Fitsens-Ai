...
with tabs[3]:
    ...
    st.write("Determine body type by height/weight or upload a full-body photo for ratio-based classification.")

    def get_detailed_plan(body_type):
        plans = {
             "Ectomorph": {"diet": "Their diet should be high in calories and protein, including foods like eggs, rice, chicken or paneer, oats, nuts, and milk to promote muscle gain. Eating every 3–4 hours, staying hydrated, and getting 7–8 hours of sleep daily help optimize growth and recovery.", "workout": "Ectomorphs should focus on heavy compound workouts like squats, bench presses, and deadlifts 4–5 days a week, keeping reps between 8–12 and avoiding excessive cardio to prevent calorie loss. "},
            "Mesomorph": {"diet": "Their diet should include a moderate calorie intake with balanced macros — lean proteins (chicken, fish, eggs), complex carbs (brown rice, oats), and healthy fats (nuts, olive oil). Staying consistent with workouts, maintaining portion control, and ensuring proper rest helps them build muscle while keeping body fat in check.", "workout": "Mesomorphs should combine strength training and moderate cardio 5 days a week, focusing on both compound lifts like squats and bench presses and accessory exercises for balance."},
            "Endomorph": {"diet": "Their diet should be calorie-controlled, high in protein, moderate in healthy fats, and low in refined carbs — including foods like eggs, lean meats, vegetables, nuts, and whole grains. Eating smaller, frequent meals, avoiding sugar, and staying consistent with workouts helps them stay lean and build muscle efficiently..", "workout": "Endomorphs should focus on a mix of strength training and higher-intensity cardio 5–6 days a week to boost metabolism and reduce body fat, emphasizing compound movements like deadlifts, squats, and HIIT sessions."}        }
        return plans.get(body_type, {"diet": "No plan", "workout": "No plan"})
    ...
    def analyze_image_body(img_bgr):
        with mp.solutions.pose.Pose(static_image_mode=True) as p:
            res = p.process(cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB))
            if not res.pose_landmarks:
                return "Unknown", None
            lm = res.pose_landmarks.landmark
            shoulder_width = abs(lm[mp.solutions.pose.PoseLandmark.LEFT_SHOULDER].x - lm[mp.solutions.pose.PoseLandmark.RIGHT_SHOULDER].x)
            hip_width = abs(lm[mp.solutions.pose.PoseLandmark.LEFT_HIP].x - lm[mp.solutions.pose.PoseLandmark.RIGHT_HIP].x)
            ratio = shoulder_width / hip_width if hip_width > 0 else 0
            if ratio > 1.60:
                return "Mesomorph", ratio
            elif 1.20 <= ratio <= 1.60:
                return "Endomorph", ratio
            else:
                return "Ectomorph", ratio

    mode = st.radio("Mode", ["Select", "Height & Weight Input", "Photo Upload"])
    if mode == "Height & Weight Input":
        h = st.number_input("Height (cm)", min_value=100, max_value=250, value=170)
        w = st.number_input("Weight (kg)", min_value=30, max_value=200, value=70)
        if st.button("Get Plan (from BMI)"):
            bt = classify_body_type(h, w)
            plan = get_detailed_plan(bt)
            st.success(f"Predicted Body Type: {bt}")
            st.markdown(f"**Diet:** {plan['diet']}")
            st.markdown(f"**Workout:** {plan['workout']}")
    elif mode == "Photo Upload":
        up = st.file_uploader("Upload full-body photo", type=["jpg","jpeg","png"])
        if up is not None:
            img = Image.open(up).convert("RGB")
            st.image(img, caption="Uploaded photo", use_column_width=True)
            if st.button("Analyze Photo"):
                img_bgr = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
                bt, ratio = analyze_image_body(img_bgr)
                if bt != "Unknown":
                    plan = get_detailed_plan(bt)
                    st.success(f"Detected Body Type: {bt}")
                    st.markdown(f"**Diet:** {plan['diet']}")
                    st.markdown(f"**Workout:** {plan['workout']}")
                    st.info(f"Shoulder/Hip ratio: {ratio:.2f}")
                else:
                    st.error("Could not detect pose. Try a clearer full-body image with good lighting.")


with tabs[4]:
    st.header("💬 Fitness & Nutrition Chatbot")
    st.write("Ask FitSens-Ai – the assistant is powered by CodeBusters")

    # load data/model lazily (with spinner)
    with st.spinner("Loading chatbot model & data..."):
        try:
            df = load_data()
            model = load_model()
            q_list = df['question'].astype(str).tolist()
            a_list = df['answer'].astype(str).tolist()
            q_embeddings = compute_embeddings(q_list, _model=model)
        except Exception as e:
            st.error("Chatbot resources failed to load. Check CSV/model availability.")
            q_list, a_list, q_embeddings, model = [], [], None, None

    query = st.text_input("Ask a fitness / nutrition question")
    if st.button("Ask") and query:
        if model is None:
            st.error("Chatbot unavailable.")
        else:
            with st.spinner("Finding best answer..."):
                ans = get_best_answer(query, q_list, a_list, q_embeddings, model)
            st.markdown(f"**Answer:** {ans}")
            if enable_tts:
                speak_text_nonblocking(ans)
